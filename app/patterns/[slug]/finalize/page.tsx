...
export default function FinalizePattern() {
  ...
  const [patternName, setPatternName] = useState<string>("Pattern");
  const [imageDataUrl, setImageDataUrl] = useState<string>("");

  // Fetch CMS pattern by slug
  useEffect(() => {
    (async () => {
      try {
        ...
        if (rec?.name) setPatternName(rec.name);
      } catch {}
    })();
  }, [slug]);
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    form.append("patternName", patternName);
      ...
      <Section title="Finalize" intro="Review your design and send it with your inquiry.">
        <div className="grid lg:grid-cols-[420px_1fr] gap-8 items-start">
          ...
          <div className="bg-[var(--card-bg)] rounded-xl border border-[color:var(--brand-taupe)]/30 p-4 shadow-[0_4px_16px_rgba(0,0,0,0.05)]">
            <div className="text-sm text-[var(--text-muted)] mb-2">{patternName} · Colors: {colors.slice(0,3).join(", ")}</div>
